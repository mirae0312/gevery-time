...
<% 
	...
	String no = (String) request.getAttribute("codeN");
...
<% } %>
		...
		<div id="body-description">
			<div class="description-wrapper">
				...
				<textarea name="bodyContent" id="bodyContent" cols="30" rows="10"><%= info.getBodyContents() %></textarea><br />				
			</div>
...
<script>
const $frm = $(document.infoEnrollFrm);

// 수정
const submitInfo = () => {
	$frm.attr("action", "<%= request.getContextPath() %>/info/modifyMain")
		.submit();
};

// 수정 취소
const backInfo = () => {
	$frm.attr("action", "<%= request.getContextPath() %>/info/view?code=<%= info.getCode() %>")
		.submit();
};
